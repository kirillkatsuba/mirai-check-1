...
from lalamo.speculator.gumbel_main import GumbelSpeculator 
...
@speculator_app.command(help="Train a speculator (Gumbel-Max trick with Coupling) from inference traces")
def train(
    trace_path: Annotated[
        Path,
        Argument(
...
def test(
    ...
    with open(speculator_path, "rb") as fd:
        speculator = GumbelSpeculator.deserialize(fd.read())
